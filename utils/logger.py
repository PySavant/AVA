...
'''
    One issue with this logging module is the restrictions on filepaths
    When using the same instance and passing it around, there will be no issues.
    However, when you create multiple instances for different sections, you have a chance to collide

    The solution is to pass different `path` arguments to each instance, creating unique log files
    The risk of collision is now minimal, but never zero because of the `errors` file.

    Modification to this system will come at a later date, for now it is simply some working code I
    already had working for a separate project.
'''
...
class LoggerModule():
    ...
    def __init__(self, level: int, *, path: str = "events"):
        init()
...
def getLogger(level: str, *, filepath: str) -> LoggerModule:
    try:
        log_level = levels[level]
    except KeyError:
        log_level = levels['TRACE']

    return LoggerModule(log_level, path=filepath)
